...
public class FisurasController : ControllerBase
{
    ...
    private static FisuraResponse MapToResponse(Fisura f)
    {
        var lastMedicion = f.Mediciones?.OrderByDescending(m => m.Fecha).FirstOrDefault();
        return new FisuraResponse(
            ...
            DeltaPorcentaje: lastMedicion?.DeltaPorcentaje,
            EsCritica: lastMedicion?.EsCritica ?? false
        );
    }
