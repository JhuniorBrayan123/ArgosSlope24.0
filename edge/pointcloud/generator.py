...
class PointCloudGenerator:
    ...
    def __init__(
        ...
        # ── Camera intrinsics (FIX 3: heuristic para cámara sin calibrar) ──
        # Para ACER HD User Facing 1280×720: fx ≈ ancho * 1.1 ≈ 1408
        # Esta heurística reemplaza el cálculo por FOV (que daba fx≈914
        # con FOV=70°) porque produce una proyección 3D más realista.
        #
        # Para máxima precisión: calibrar con tablero de ajedrez usando
        # calibrate_camera_interactive() y reemplazar estos valores.
        self._fx = float(self._width * 1.1)
        self._fy = self._fx  # píxeles cuadrados
        self._cx = float(self._width) / 2.0
        self._cy = float(self._height) / 2.0
        self._max_depth_m = 5.0  # FIX 2: filtrar puntos > 5m
