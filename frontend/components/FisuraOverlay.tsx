...
interface FisuraDetectada {
  ...
  width: number;
  height: number;
  // Nuevos campos unificados (largo/ancho/area) — el Edge envía estos desde v2.0
  largo: number;
  ancho: number;
  area: number;
  unidad: string;          // 'px' | 'mm'
  calibrado: boolean;
  origen: string;          // 'real' | 'demo'
...
export default function FisuraOverlay({
  ...
  const draw = useCallback(() => {
    ...
    for (const f of fisuras) {
      ...
      // Determinar cómo mostrar la medida (con unidad correcta)
      const unidad = f.unidad || 'px';
      const esMm = unidad === 'mm';
      const largoVal = (f.largo ?? 0).toFixed(esMm ? 2 : 0);
      const anchoVal = (f.ancho ?? 0).toFixed(esMm ? 2 : 0);
      const medida = esMm
        ? `L:${largoVal}mm A:${anchoVal}mm${f.calibrado ? '' : ' ~est'}`
        : `L:${largoVal}px A:${anchoVal}px`;
